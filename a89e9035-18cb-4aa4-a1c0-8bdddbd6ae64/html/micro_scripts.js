...
/**
 * Detect long titles that might wrap to multiple lines and add responsive spacing
 */
function setupResponsiveTitleSpacing() {
    const titles = document.querySelectorAll('.micro-title');
    
    titles.forEach(title => {
        const titleText = title.textContent || title.innerText;
        const titleLength = titleText.length;
        
        // Detect potentially long titles (rough estimation)
        // Adjust threshold based on typical mobile screen width and font size
        const isMobile = window.innerWidth <= 768;
        const longTitleThreshold = isMobile ? 30 : 50; // Characters that typically cause wrapping
        
        if (titleLength > longTitleThreshold) {
            title.setAttribute('data-long-title', 'true');
        } else {
            title.removeAttribute('data-long-title');
        }
    });
}

// Call this function when window resizes to adjust for different screen sizes
window.addEventListener('resize', setupResponsiveTitleSpacing);
...
/**
 * MERMAID DIAGRAMS - INITIALIZATION AND RENDERING
 */

// Improved Mermaid initialization with error handling and debugging
...
document.addEventListener('DOMContentLoaded', function() {
    initializeSlides();
    setupResponsiveTitleSpacing(); // Setup responsive spacing for long titles
